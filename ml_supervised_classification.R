...
# Master ESDEN: Supervised Classification
##########################################################################

# Packages
list.of.packages <- c("tidyverse", "modelr", "broom", "ISLR", "ROCR", "caret")
...
hist(predicted_value)
predicted_class <- ifelse(predicted_value>0.5, "Yes","No")
performance_data<-data.frame(observed=test$default,
                             predicted= predicted_class)

# Error de clasificación (misclassification rate)
round(mean(performance_data$observed!=performance_data$predicted),4) 
...
caret::confusionMatrix(as.factor(predicted_class), test$default, mode="prec_recall",positive="Yes")
